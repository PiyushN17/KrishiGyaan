...
async function generateAiResponse(prompt) {
  let primaryError;
  try {
    return await generateWithGroq(prompt, CONFIG.groqKey, CONFIG.groqModel, "groq-primary");
  } catch (error) {
    primaryError = error;
  }

  try {
    return await generateWithGroq(prompt, CONFIG.groqKey2, CONFIG.groqModel2, "groq-fallback");
  } catch (fallbackError) {
