...
const kgGeminiCache = JSON.parse(localStorage.getItem("krishigyaanGeminiCache") || "{}");
...
function kgCurrentLanguageLabel() {
  return KG_LANGUAGES[kgActiveLanguage]?.label || "English";
}

function kgAiLanguageGuard() {
  const label = kgCurrentLanguageLabel();
  return [
    `Selected website language: ${label} (${kgActiveLanguage}).`,
...
async function kgAiText(prompt) {
  const guardedPrompt = `${kgAiLanguageGuard()}\n\n${prompt}`;
  const cacheKey = `ai:${kgActiveLanguage}:${kgHashText(guardedPrompt)}`;
  if (kgGeminiCache[cacheKey]) return kgGeminiCache[cacheKey];
  ...
  if (text) {
    kgGeminiCache[cacheKey] = text;
    localStorage.setItem("krishigyaanGeminiCache", JSON.stringify(kgGeminiCache));
  }
  return text;
}
...
async function kgTranslateMissingPageText(lang) {
  ...
  try {
    ...
    console.warn("Gemini translation fallback failed:", error);
  }
}
...
async function kgTranslateLooseUiText(lang) {
  ...
  try {
    ...
    console.warn("Gemini loose UI translation failed:", error);
  }
}
...
function kgSpeak(text, lang = kgActiveLanguage) {
  ...
  utterance.lang = KG_LANGUAGES[lang]?.speech || "en-IN";
  utterance.rate = 0.9;
  utterance.pitch = 1;
...
function kgInitShared({ askLocation = true } = {}) {
  ...
  document.addEventListener("click", (event) => {
    const target = event.target.closest(".speakable, [data-i18n]");
    if (!target || target.closest(".voice-panel") || target.closest("input, textarea, select")) return;
    const text = kgCollectReadableText(target);
